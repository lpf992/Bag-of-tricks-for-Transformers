...
class Hyperparameters:
    ...
    # trick: muon-ema-update-smoothing — EMA applied to Muon orthogonalized updates
    muon_update_smoothing_start = float(os.environ.get("MUON_UPDATE_SMOOTHING_START", 0.5))
    muon_update_smoothing_end = float(os.environ.get("MUON_UPDATE_SMOOTHING_END", 0.2))
    muon_update_smoothing_warmup_steps = int(os.environ.get("MUON_UPDATE_SMOOTHING_WARMUP_STEPS", 3000))
...
class Muon(torch.optim.Optimizer):
    # trick: muon-ema-update-smoothing — add update_smoothing parameter
    def __init__(self, params, lr: float, momentum: float, backend_steps: int, nesterov: bool = True, update_smoothing: float = 0.0):
        super().__init__(
            params,
            dict(lr=lr, momentum=momentum, backend_steps=backend_steps, nesterov=nesterov, update_smoothing=update_smoothing),
        )
            ...
            for i, p in enumerate(params):
                if i % world_size == rank and p.grad is not None:
                    ...
                    if "momentum_buffer" not in state:
                        ...
                        # trick: muon-ema-update-smoothing — buffer for smoothed updates
                        state["smoothed_update_buffer"] = torch.zeros_like(g, dtype=torch.bfloat16)
                    ...
                    # trick: muon-ema-update-smoothing — apply EMA to the orthogonalized update
                    update_smoothing = group["update_smoothing"]
                    if update_smoothing > 0:
                        sbuf = state["smoothed_update_buffer"]
                        sbuf.mul_(update_smoothing).add_(g, alpha=(1 - update_smoothing))
                        g = update_smoothing * sbuf + (1 - update_smoothing) * g
...
def main() -> None:
    ...
    # trick: muon-ema-update-smoothing — pass initial smoothing weight
    optimizer_muon = Muon(
        matrix_params,
        lr=args.matrix_lr,
        momentum=args.muon_momentum,
        backend_steps=args.muon_backend_steps,
        update_smoothing=args.muon_update_smoothing_start,
    )
    ...
    while True:
        ...
        # trick: muon-ema-update-smoothing — decay update smoothing from start to end
        if args.muon_update_smoothing_warmup_steps > 0:
            smoothing_frac = min(step / args.muon_update_smoothing_warmup_steps, 1.0)
        else:
            smoothing_frac = 1.0
        muon_update_smoothing = (1 - smoothing_frac) * args.muon_update_smoothing_start + smoothing_frac * args.muon_update_smoothing_end
        for group in optimizer_muon.param_groups:
            group["update_smoothing"] = muon_update_smoothing
            ...
            wandb_log(
                {
                    ...
                    "train/muon_update_smoothing": float(muon_update_smoothing),  # trick: muon-ema-update-smoothing
